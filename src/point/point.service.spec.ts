...
describe('PointService', () => {
  ...
  describe('포인트 조회', () => {
    ...
    it('undefined로 포인트 조회시 에러 반환', async () => {
      userPointTable.selectById.mockRejectedValue(new Error('올바르지 않은 ID 값 입니다.'));
      await expect(pointService.getPoint(undefined)).rejects.toThrow('올바르지 않은 ID 값 입니다.');
    });

    it('null로 포인트 조회시 에러 반환', async () => {
      userPointTable.selectById.mockRejectedValue(new Error('올바르지 않은 ID 값 입니다.'));
      await expect(pointService.getPoint(null)).rejects.toThrow('올바르지 않은 ID 값 입니다.');
    });
  });

  describe('포인트 충전', () => {
    it('포인트에 0을 충전시 에러 반환', async () => {
      await expect(pointService.chargePoint(1, 0)).rejects.toThrow('포인트에 0을 충전할 수 없다');
    });

    it('포인트에 음수로 충전시 에러 반환', async () => {
      await expect(pointService.chargePoint(1, -20)).rejects.toThrow('음수로 충전시 예외발생');
    });
  ...
  describe('포인트 사용', () => {
    ...
    it('0 포인트로 충전시 에러 반환', async () => {
      await expect(pointService.usePoint(1, 0)).rejects.toThrow('0으로 포인트를 사용시 예외 발생');
    });
    ...
    it('실수인 Id로 포인트 조회시 에러 반환', async () => {
      ...
      await expect(pointService.usePoint(1.1, 100)).rejects.toThrow('올바르지 않은 ID 값 입니다.');
    });
    ...
    it('잔고 부족시 포인트 사용 실패', async () => {
      ...
      await expect(pointService.usePoint(1, 100)).rejects.toThrow('잔고 부족, 포인트 사용 실패');
    });
  });
  ...
  describe('포인트 충전/이용 내역 조회', () => {
    ...
    it('음수의 Id로 포인트 조회시 에러 반환', async () => {
      await expect(pointService.getHistory(-1)).rejects.toThrow('올바르지 않은 ID 값 입니다.');
    });
    ...
    it('실수인 Id로 포인트 조회시 에러 반환', async () => {
      await expect(pointService.getHistory(1.1)).rejects.toThrow('올바르지 않은 ID 값 입니다.');
    });
  });
});
